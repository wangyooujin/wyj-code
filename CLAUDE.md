...
| `crates/store` | `wyj-store` | MCP/Skill/Plugin 配置管理数据层：安装元数据 lockfile、MCP registry HTTP client、skill/plugin git marketplace client、install/upgrade/uninstall/enable 编排；只管配置写入（config.toml/`.wyj-code/mcp.toml`/lockfile），绝不 shell out 执行依赖安装（如 `npm install`）；`import` 模块承载 `/import` 与 `extensions migrate` 的 scan/apply；`schedule`/`cron_sync` 模块承载定时任务数据与系统 crontab 同步（这里是本 crate 唯一会 shell out 执行系统命令的例外，见下方"定时任务系统"节）|
...
9. **统一 Extensions 资源平台**（`wyj-store::extensions` + `/extensions` + `wyj-code extensions`）：统一读取 Skill/MCP/Plugin lockfile 投影，lockfile v2 新增跨类型 `extensions` 索引，同时兼容旧 v1 数组；支持 `list`/`doctor`/`migrate`/`install`/`upgrade`/`enable`/`disable`/`remove` 和 `--json`。项目原生路径优先于项目插件和 wyj 自有路径，全局同理。**一键导入**（v1.4，`wyj-store::import` + TUI `/import` + CLI `extensions migrate --from codex|claude|all [--dry-run]`）：扫描 Codex（`~/.codex/config.toml` 的 `[mcp_servers.*]`、`~/.codex/prompts/*.md`）与 Claude Code（`~/.claude.json`/`.mcp.json` 的 `mcpServers`、`~/.claude/commands`/`.claude/commands`、`~/.claude/agents`/`.claude/agents`）的配置，物化复制成 wyj 自管配置（MCP 进 config.toml/`.wyj-code/mcp.toml`，skill/agent 复制进对应 `skills/`/`agents/` 目录），原文件永远只读保留。TUI 面板默认勾选全部无冲突项（同名不同内容标 conflict、默认不勾、勾选即覆盖；内容完全相同的候选不产出，天然幂等），来自 Claude commands/agents 目录的导入副本会被在线原文件遮蔽（合并链真 CC 路径优先），报告中逐条提示。冲突检测与写回必须走 `Config::load_file_only()`（裸读 config.toml），不能用 `Config::load()`——后者合并 `~/.claude.json` 会导致全量误报冲突并把原生条目误物化写盘（旧 `extensions::migrate` 曾因此有 bug）。CLI 非交互语义：冲突项一律跳过并列出。MCP 支持 stdio 与 Streamable HTTP（URL/环境变量引用 headers），工具名稳定映射为 `mcp__<server>__<tool>`。Skill loader 递归支持 `namespace:name` 命令。插件仍通过 `.claude-plugin` manifest 和 marketplace 安装；commands/skills、agents、MCP contributions 是 v1.2.2 支持边界，hooks/themes 仍未纳入。资源变更的持久化状态在下一次 Agent 回合边界生效，CLI 输出会明确提示这一点。
...
13. **定时任务系统**（v1.4，`store::schedule`/`store::cron_sync` + `/schedule` + `wyj-code schedule`）：wyj-code 本身**没有常驻后台进程**（`-p` 单次执行完就退出），定时能力完全依赖系统级 `crontab`（v1 只支持 macOS/Linux，Windows 面板可管理任务但同步动作报不支持）唤起 headless 执行，架构上分三层：
