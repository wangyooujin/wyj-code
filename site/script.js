(function () {
  ...
  var translations = {
    zh: {
      ...
      'changelog.latest': '最新',
      'changelog.v151': '修复国内模型一次返回多个完整工具调用时的兼容路径：保守能力仍控制串行执行，但不再错误拒绝额外合法调用或误触参数重试熔断；同时修复 Linux 专属严格 Clippy 阻塞，并以不可移动的新补丁版本完成发布。',
    ...
    en: {
      ...
      'changelog.v151': 'Fixed compatibility when a Chinese model emits multiple complete tool calls in one response: conservative capabilities still serialize execution, but valid extra calls are no longer rejected or counted as argument-retry failures. Also fixed Linux-only strict Clippy blockers and republished under a new immutable patch version.',
