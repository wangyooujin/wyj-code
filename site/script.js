(function () {
  ...
  var translations = {
    zh: {
      ...
      'delivery.evolution.title': 'v1.5.12 · TUI 终端 panic 还原 + char-boundary 安全',
      'delivery.evolution.desc': '新增 `crates/tui/src/panic_guard.rs` 进程级 `panic::set_hook`：`cli::main()` 入口最前 `wyj_tui::panic_guard::install()`（`take_hook` 链式保留前一个 hook），`run_tui` 在 `enter/leave_terminal_screen` 配对 `mark_active()/mark_inactive()`（全局 `AtomicBool TUI_SCREEN_ACTIVE`），panic 触发时 hook 检查到 active 就 best-effort 还原终端（`DisableMouseCapture` → `LeaveAlternateScreen` → `disable_raw_mode` → `Show`，每步吞错）再调 prev hook 写 panic 信息。新增 `crates/core/src/textutil.rs::floor_char_boundary(&str, usize) -> usize`：按字节切字符串时先回退到最近 char boundary 再 `String::truncate`，治本 `memory_v3.rs` 拼装 Active Memory 上下文超过 `MAX_CONTEXT_BYTES` 时撞 CJK/emoji char boundary 触发的 `is_char_boundary` assertion panic。配套新增 `TestBackend` 回归测试 `draw_input_keeps_cursor_visible_while_thinking` 与 `draw_input_title_shows_thinking_spinner_when_thinking` 防止"is_thinking 时跳过 `set_cursor_position` 致光标卡死"再次回归。',
    ...
    en: {
      ...
      'delivery.evolution.title': 'v1.5.12 · TUI panic-safe terminal + char-boundary safety',
      'delivery.evolution.desc': 'A new `crates/tui/src/panic_guard.rs` installs a process-level `panic::set_hook`: `wyj_tui::panic_guard::install()` runs at the top of `cli::main()` (chained via `take_hook` so the previous hook still fires), `run_tui` pairs `mark_active()` / `mark_inactive()` around `enter/leave_terminal_screen` via a global `AtomicBool TUI_SCREEN_ACTIVE`. When the hook fires and the alternate screen is active, it best-effort restores the terminal (`DisableMouseCapture` → `LeaveAlternateScreen` → `disable_raw_mode` → `Show`, errors swallowed) before calling the previous hook. A new `crates/core/src/textutil.rs::floor_char_boundary(&str, usize) -> usize` rewinds a byte index to the nearest char boundary before `String::truncate`, curing the `is_char_boundary` assertion panic that `memory_v3.rs` triggered on CJK / emoji content above `MAX_CONTEXT_BYTES`. Two `TestBackend` regression tests (`draw_input_keeps_cursor_visible_while_thinking`, `draw_input_title_shows_thinking_spinner_when_thinking`) lock in the cursor-position fix.',
