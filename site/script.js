(function () {
  ...
  var translations = {
    zh: {
      ...
      'hero.desc': 'v1.5.6 已公开发布：Memory v3 收敛为 Global/Project 两层、AI 自动管理项目记忆、Task 续接与“继续”语义、/memory clear-all 一键清空重建；安装脚本会获取最新公开 Release。',
      ...
      'delivery.title': 'v1.5.6 · Memory v3 最终设计',
      'delivery.desc': 'v1.5.6 已公开交付 Memory v3 收敛为 Global/Project 两层、AI 自动管理项目记忆、Task 续接与“继续”语义、`/memory clear-all` 一键清空重建；历史 tag 保持不可移动，Release 与安装脚本同步指向最新公开版本。',
      'delivery.status': '已交付',
      ...
      'delivery.evolution.title': 'v1.5.6 · Memory v3 单一数据面',
      'delivery.evolution.desc': 'v3 收敛为 Global/Project 两层（删除共享 Workspace scope），AI 自动管理项目记忆、Project 覆盖 Global 冲突；Global 背景提取走 Pending 候选，需模型用工具三步自然语言确认；新增 Task 类型 + 动态 Project Brief，“继续/接着/resume/go on” 命中即恢复最近 InProgress Task；`/memory clear-all` 一键清空重建，旧记录保留时间戳备份。Evolution 收敛到 GovernanceOnly，普通 Memory 数据层不再被注入也不再自动生成。',
      ...
      'delivery.release': '查看 v1.5.6 Release →',
    ...
    en: {
      ...
      'hero.desc': 'v1.5.6 is now public: Memory v3 collapses to Global/Project two scopes, the AI auto-manages project memory, Task continuity plus the “continue” keyword resume, and `/memory clear-all` rebuilds the store in one shot. Installers fetch the latest public Release.',
      ...
      'delivery.kicker': 'Delivery Status',
      'delivery.title': 'v1.5.6 · Memory v3 final design',
      'delivery.desc': 'v1.5.6 publicly delivers Memory v3 collapsing to Global/Project two scopes, AI auto-managing project memory, Task continuity plus the “continue” keyword resume, and `/memory clear-all` rebuilding the store in one shot. Historical tags remain immutable, while Release links and installers point to the latest public version.',
      ...
      'delivery.evolution.title': 'v1.5.6 · Single Memory v3 data plane',
      'delivery.evolution.desc': 'v3 collapses to Global/Project two scopes (the shared Workspace scope is removed); AI auto-manages project memory and Project overrides Global on conflict. Global background extractions land as Pending candidates and require the model to confirm them via three natural-language tool steps. New Task kind plus a dynamic Project Brief lets “continue / resume / go on” resume the most recent InProgress task; `/memory clear-all` rebuilds the store in one shot and timestamps the backup. Evolution collapses to GovernanceOnly, so the plain Memory layer is no longer injected or auto-generated.',
      ...
      'delivery.release': 'View the v1.5.6 Release →',
