(function () {
  ...
  var translations = {
    zh: {
      ...
      'hero.term.run': '运行 <span class="text-paper">cargo test -p wyj-cli</span>',
      ...
      'arch.title': '11 个 crate 的 Rust workspace',
      ...
      'changelog.latest': '最新',
      'changelog.v142': 'TUI 对齐 Codex 静态执行流：Thinking、工具结果和 Bash 输出默认最多显示 3 个视觉行，Edit/Write 自动展示彩色 diff；移除聊天消息选中、展开与 Ctrl+O，新增 Shift+↑ 显式进入内容区、聊天逐行滚动、Todo 单任务详情，以及上滚阅读时不抢视口的新消息提示。',
    ...
    en: {
      ...
      'hero.term.run': 'Running <span class="text-paper">cargo test -p wyj-cli</span>',
      'hero.term.testresult': 'test result: ok. 12 passed',
      ...
      'arch.kicker': 'Architecture',
      'arch.title': 'An 11-crate Rust workspace',
      ...
      'changelog.v142': 'Aligned the TUI with a Codex-style static execution stream: thinking, tool results, and Bash output show at most three visual rows by default, while Edit/Write render a colored diff automatically. Removed chat-message selection, expansion, and Ctrl+O; added explicit Shift+↑ content focus, line-by-line chat scrolling, per-task Todo detail, and non-disruptive new-message notices while reading history.',
