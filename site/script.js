(function () {
  ...
  var translations = {
    zh: {
      ...
      'hero.desc': 'v1.5.11 已公开发布：Session 存储 CAS+Delta 重构（单 checkpoint 从 11MB 降至 ~100KB、整体会话占用降 ~99%）、/new 一键开新会话对齐 Claude Code 语义、`wyj-code storage status` 占用诊断；安装脚本会获取最新公开 Release。',
      ...
      'delivery.title': 'v1.5.11 · Session 存储 CAS 重构 + /new 命令',
      'delivery.desc': 'v1.5.11 公开交付 Session 存储重构：sha256 内容寻址 CAS Blob Pool 把 256 文件 ×21 checkpoint = 5376 份冗余压缩为 256 份真实内容（单 checkpoint 从 11MB → ~100KB）；同 cwd 相邻 checkpoint 自动写 Delta 快照，父链最长 20 层；超大 base64 image / 长 thinking 自动外置到 CAS，session.json 同步瘦身。新增 `/new` slash 命令对齐 Claude Code 新会话语义——自动保存当前会话后分配新 session_id、清空 TUI 状态、无二次确认弹窗；新增 `wyj-code storage {status,doctor,prune}` 占用治理子命令。历史 tag 保持不可移动，Release 与安装脚本同步指向最新公开版本。',
      'delivery.status': '已交付',
      ...
      'delivery.evolution.title': 'v1.5.11 · Session 存储重构 (CAS + Delta)',
      'delivery.evolution.desc': '新增 `crates/core/src/workspace_cas.rs` 提供 sha256 内容寻址 Blob Pool：`intern()/get()/release()/gc()/stats()`，CAS 路径 `~/.wyj-code/cas/sha256/aa/bb/<hash>.blob + .meta.json`。`FileEntry { hash, inline_bytes, size, sha256 }` 替代内联字节，serde 用 `#[serde(default, alias = "bytes")]` 兼容旧 v1.5.10 落盘。`WorkspaceSnapshot::Delta(DeltaSnapshot)` 同 cwd 自动 fold 父链（最多 20 层），跨 cwd 强制 baseline。`externalize_block_with` 把 >32KB image 与 >16KB thinking 外置，`cas://<hash>` 引用 + `materialize_block_with` 在 resume 时还原。`gc()` 用 `last_ref_at` LRU 淘汰 0-ref blob，对齐 git pack-files 语义。实测：长会话从 1.15GB 降至 ~3MB（~99.5% 压缩率）。',
      ...
      'delivery.release': '查看 v1.5.11 Release →',
    ...
    en: {
      ...
      'hero.desc': 'v1.5.11 is now public: Session storage CAS + Delta refactor (single checkpoint shrinks from ~11MB to ~100KB, overall session footprint drops ~99%), `/new` slash command aligns with Claude Code\'s new-session semantics, and `wyj-code storage status` reports subsystem occupancy. Installers fetch the latest public Release.',
      ...
      'delivery.kicker': 'Delivery Status',
      'delivery.title': 'v1.5.11 · Session storage CAS refactor + /new command',
      'delivery.desc': 'v1.5.11 publicly delivers a Session storage refactor: a sha256 content-addressed CAS Blob Pool collapses 256 files × 21 checkpoints (5,376 redundant copies) into 256 unique blobs (single checkpoint shrinks from ~11MB to ~100KB). Adjacent checkpoints in the same cwd auto-write Delta snapshots, with the parent chain capped at 20 levels. Oversized base64 images and long thinking blocks are externalized to CAS, slimming session.json at the same time. The new `/new` slash command matches Claude Code\'s new-session semantics — auto-save the current session, allocate a new session_id, clear TUI state, no second confirmation prompt. The new `wyj-code storage {status,doctor,prune}` subcommand covers occupancy governance; `status --json` is script-friendly. Historical tags remain immutable, while Release links and installers point to the latest public version.',
      ...
      'delivery.evolution.title': 'v1.5.11 · Session storage refactor (CAS + Delta)',
      'delivery.evolution.desc': 'A new `crates/core/src/workspace_cas.rs` provides a sha256 content-addressed Blob Pool (`intern()/get()/release()/gc()/stats()`); CAS layout is `~/.wyj-code/cas/sha256/aa/bb/<hash>.blob + .meta.json`. `FileEntry { hash, inline_bytes, size, sha256 }` replaces the inlined bytes; `#[serde(default, alias = "bytes")]` keeps v1.5.10 checkpoints loadable. `WorkspaceSnapshot::Delta(DeltaSnapshot)` auto-folds the parent chain when cwd matches (capped at 20 levels); cross-cwd checkpoints force a fresh baseline. `externalize_block_with` ships images >32KB and thinking blocks >16KB into CAS (`cas://<hash>` reference + `materialize_block_with` rehydrates on resume). `gc()` evicts zero-ref blobs by `last_ref_at` LRU, mirroring git\'s pack-file semantics. Real-world effect: a long session goes from 1.15GB to ~3MB (~99.5% compression).',
      ...
      'delivery.release': 'View the v1.5.11 Release →',
