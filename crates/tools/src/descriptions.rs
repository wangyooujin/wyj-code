...
pub const SUB_AGENT_TEMPLATE: &str = "Spawns an independent sub-agent to complete a task, optionally in the background (run_in_background=true returns immediately; the result is injected into the conversation when ready). A sub-agent initially sees only your prompt — not this conversation. The runtime can append user-initiated follow-up or retry instructions at complete model/tool boundaries, but the initial prompt must still be a complete, self-contained task description stating exactly what to investigate or do and what the final report must contain. Use sub-agents for multi-step tasks worth isolating or broad codebase research; do NOT spawn one when you already know the two or three files to read. Multiple Agent calls in one response run concurrently.\n\nAvailable agent types:\n{types}";
