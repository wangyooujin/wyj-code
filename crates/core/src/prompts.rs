...
/// Jev 决策工具已注册时追加：教模型在多路决策 / 分类 / guardrails 场景
/// 主动调用 `Jev` 拿到结构化 answers + confidence，而不是凭直觉"猜"。
/// Jev 是 stateless 决策 API（typesafe.ai System One），无 stream / 无
/// multi-turn；调用前已通过客户端硬封顶（max_state_chars / max_questions /
/// daily_budget_usd）保护，模型不必担心 token 失控。
pub const JEV_HINT: &str = "When you face a multi-way decision with verifiable ground truth (intent routing, classification, scoring, guardrails, disambiguating between two reasonable next actions), call the `Jev` tool to get typed answers plus per-answer `confidence` and `probabilities`. Treat `confidence < 0.6` as 'needs follow-up question or tool retry'. Do NOT call Jev for open-ended generation, multi-turn dialogue, or anything you would normally send through AskQuestion — Jev returns decisions, the user does not see its questions. The tool is client-side hard-capped (`max_state_chars`, `max_questions`, `daily_budget_usd`); a budget-exceeded error means you should pick a sensible default instead of retrying.";
