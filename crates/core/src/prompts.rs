...
/// an explicit compatibility exception, never an implicit fallback.
pub const COMPUTER_USE_HINT: &str = "For desktop GUI work, preserve the human user's mouse, keyboard focus, and foreground application. On macOS, ALWAYS prefer this workflow: (1) call `window_capture` action `list`, (2) capture the exact `window_id`, then (3) use `app_computer` with that `window_id`, its `generation`, and captured target dimensions. `app_computer` uses background Accessibility/target-PID actions and never silently falls back to global input. After `target_changed`, capture again. After `requires_foreground_takeover`, `preempted_by_user`, `user_active`, `input_monitor_unavailable`, `screen_locked`, or a permission error, STOP and report the condition; do not retry automatically and do not switch to the legacy `computer` tool on your own. The `computer` tool is foreground compatibility mode: its mutations may be disabled by configuration, require an exact input monitor and a fresh one-use screenshot observation, and can visibly occupy the user's pointer/focus. Use it only when the requested operation truly cannot be expressed by `app_computer` and foreground takeover has been explicitly enabled. To launch an application, use Bash instead of hunting through the GUI (macOS: `open -a \"App Name\"`; Windows: `start \"\" \"App Name\"`), wait briefly, then discover/capture its window. Full-desktop and window screenshots can downscale small text; use the legacy `zoom` action only for precise read-only crops when needed, never guess numbers. Reading the user's own on-screen messages, documents, or notifications needs no app API and is an ordinary screenshot task.";
